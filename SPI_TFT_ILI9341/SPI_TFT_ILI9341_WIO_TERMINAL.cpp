...
#if defined(TARGET_WIO_TERMINAL)

#define FORCE_8BIT_TRANSFER
...
#if defined(FORCE_8BIT_TRANSFER)
    SPI::write((color >> 8) & 0xff);
...
void SPI_TFT_ILI9341::fillrect(int x0, int y0, int x1, int y1, int color)
{
    ...
    wr_cmd(0x2C);  // send pixel
#if defined(FORCE_8BIT_TRANSFER)
    char *txbuf = (char *)malloc(w * 2);
    for (int xp = 0; xp < w * 2; xp += 2) {
        txbuf[xp] = (color >> 8) & 0xff;
        txbuf[xp + 1] = (color & 0xff);
    }
    for (int yp = 0; yp < h; yp++) {
        while (SPI::transfer(txbuf, w * 2, NULL, 0, 8, NULL, 0)) {
            thread_sleep_for(0);
        }
    }
    free(txbuf);
...
void SPI_TFT_ILI9341::character(int x, int y, int c)
{
    ...
    char *bitmap = (char *)malloc(hor * vert * 2);
    for (j = 0; j < vert; j++) {  //  vert line
        for (i = 0; i < hor; i++) {   //  horz line
            ...
            if (( z & b ) == 0x00) {
#if defined(FORCE_8BIT_TRANSFER)
                bitmap[(j * hor + i) * 2 + 0] = (_background >> 8) & 0xff;
                bitmap[(j * hor + i) * 2 + 1] = _background & 0xff;
#else
                SPI::write(_background);
#endif
            } else {
#if defined(FORCE_8BIT_TRANSFER)
                bitmap[(j * hor + i) * 2 + 0] = (_foreground >> 8) & 0xff;
                bitmap[(j * hor + i) * 2 + 1] = _foreground & 0xff;
#else
    ...
    while (SPI::transfer(bitmap, hor * vert * 2, NULL, 0, 8, NULL, 0)) {
        thread_sleep_for(0);
    }
    free(bitmap);
...
#define OffsetBPP           28
    ...
    for (j = PixelHeigh - 1; j >= 0; j--) {               //Lines bottom up
        ...
        for (i = 0; i < PixelWidth; i++) {
            line[i] = ((line[i] & 0xff) << 8) | ((line[i] >> 8) & 0xff);
        }
        while (SPI::transfer(line, PixelWidth * 2, NULL, 0, 8, NULL, 0)) {
            thread_sleep_for(0);
        }
    }
    _cs = 1;
