    ...
    struct i2c_slave_module *const module)
{
    ...
    while ((!(i2c_hw->SERCOM_INTFLAG & SERCOM_I2CS_INTFLAG_DRDY(1))) &&
...
void i2c_init(i2c_t *obj, PinName sda, PinName scl)
{
    ...
    hw = (sercom_registers_t*)pinmap_peripheral_sercom(NC, sercom_index);
...
void i2c_frequency(i2c_t *obj, int hz)
{
    ...
    sercom_i2cm_registers_t *const i2c_module = &(pI2C_S(obj)->master.hw->I2CM);
    ...
    tmp_baud = (int32_t)(div_ceil(
                             system_gclk_chan_get_hz(chan_index[sercom_index]), (2000*(baud_rate))) - 5);
    ...
    if (tmp_status_code != STATUS_ERR_BAUDRATE_UNAVAILABLE) {
        ...
        i2c_module->SERCOM_BAUD = SERCOM_I2CM_BAUD_BAUD(tmp_baud) | SERCOM_I2CM_BAUD_HSBAUD(tmp_baud_hs);
...
int  i2c_stop(i2c_t *obj)
{
    ...
    if (pI2C_S(obj)->mode == I2C_MODE_MASTER) {
        ...
        sercom_i2cs_registers_t *const i2c_hw = &(pI2C_S(obj)->slave.hw->I2CS);
...
int  i2c_write_address(i2c_t *obj, int address, int rw_flag)
{
    ...
    i2c_module->SERCOM_ADDR = ((address & 0x7F) << 1) | (rw_flag & 0x01) | (0 << SERCOM_I2CM_ADDR_HS_Pos);
...
int  i2c_byte_read(i2c_t *obj, int last)
{
    ...
    if (pI2C_S(obj)->mode == I2C_MODE_MASTER) {
        ...
        if (last) {
            /* Set action to nack. */
            i2c_hw->SERCOM_CTRLB |= SERCOM_I2CM_CTRLB_ACKACT(1);
        } else {
            /* Set action to ack. */
            i2c_hw->SERCOM_CTRLB &= ~SERCOM_I2CM_CTRLB_ACKACT(1);
        }
...
int  i2c_slave_receive(i2c_t *obj)
{
    ...
    sercom_i2cs_registers_t *const i2c_module = &(pI2C_S(obj)->slave.hw->I2CS);

    if (i2c_module->SERCOM_INTFLAG & SERCOM_I2CS_INTFLAG_AMATCH(1)) {
        if (i2c_module->SERCOM_STATUS & SERCOM_I2CS_STATUS_DIR(1)) {
            /* Slave is read addressed */
            return 1;
        } else {
            if (!(i2c_module->SERCOM_DATA & 0xFF)) {
...
void i2c_slave_address(i2c_t *obj, int idx, uint32_t address, uint32_t mask)
{
    ...
    i2c_hw->SERCOM_ADDR = (address << SERCOM_I2CS_ADDR_ADDR_Pos) |
...
void i2c_abort_asynch(i2c_t *obj)
{
    ...
    i2c_module->SERCOM_INTENCLR = SERCOM_I2CM_INTENCLR_MB(1) | SERCOM_I2CM_INTENCLR_SB(1);
    ...
    if ((pI2C_S(obj)->master.status != STATUS_ERR_PACKET_COLLISION) &&  pI2C_S(obj)->master.send_stop) {
        _i2c_master_wait_for_sync(&pI2C_S(obj)->master);
        i2c_module->SERCOM_CTRLB |= SERCOM_I2CM_CTRLB_ACKACT(1) | SERCOM_I2CM_CTRLB_CMD(3);
    }
