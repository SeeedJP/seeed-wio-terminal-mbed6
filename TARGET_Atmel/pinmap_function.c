...
uint32_t pinmap_find_peripheral_from_pad(PinName pin, enum sercom_pad_selection pad_select)
{
    ...
    if (pad_select == SERCOM_USE_EXTENDED_PAD) {
        pin_sercom = pinmap_find_peripheral(pin, PinMap_SERCOM_Ext);
    }
    if (pin_sercom == (uint32_t)NC) {
        pin_sercom = pinmap_find_peripheral(pin, PinMap_SERCOM);
    }
...
uint32_t pinmap_function_sercom(PinName pin, uint32_t sercom_index)
{
    ...
    index = pinmap_peripheral(pin, PinMap_SERCOM);
    if ((index & 0x0F) == sercom_index) {
        func = pinmap_function(pin, PinMap_SERCOM);
        return func & 0x0F;
    }
    index = pinmap_peripheral(pin, PinMap_SERCOM_Ext);
    if ((index & 0x0F) == sercom_index) {
        func = pinmap_function(pin, PinMap_SERCOM_Ext);
        return func & 0x0F;
    }
    return (uint32_t)NC;
}
...
{
    uint32_t sercom_address[] = {
        SERCOM0_BASE_ADDRESS,
        SERCOM1_BASE_ADDRESS,
        SERCOM2_BASE_ADDRESS,
        SERCOM3_BASE_ADDRESS,
        SERCOM4_BASE_ADDRESS,
        SERCOM5_BASE_ADDRESS,
        SERCOM6_BASE_ADDRESS,
 ...
 */
uint32_t pinmap_channel_pwm(PinName pin, uint32_t pwm_index)
{
    PinMap *pwm_ch = PinMap_PWM;
