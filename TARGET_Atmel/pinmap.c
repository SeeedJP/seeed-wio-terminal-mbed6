...
static inline void pinmux_get_current_config(PinName pin, struct system_pinmux_config *const config)
{
    ...
    port_group_registers_t *const port = system_pinmux_get_group_from_gpio_pin(pin);
    MBED_ASSERT(port);

    config->mux_position = system_pinmux_pin_get_mux_position(pin);

    if (port->PORT_PINCFG[pin_index] & PORT_PINCFG_INEN(1)) {
        if (port->PORT_DIR & pin_mask) {
            config->direction = SYSTEM_PINMUX_PIN_DIR_OUTPUT_WITH_READBACK;
            config->input_pull = SYSTEM_PINMUX_PIN_PULL_NONE;
        } else {
            config->direction = SYSTEM_PINMUX_PIN_DIR_INPUT;
            if (port->PORT_PINCFG[pin_index] & PORT_PINCFG_PULLEN(1)) {
                if (port->PORT_OUT & pin_mask) {
